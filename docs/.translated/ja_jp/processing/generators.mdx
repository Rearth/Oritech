---
タイトル: 発電機
アイコン: oritech:basic_generator_block
関連アイテム: [oritech:basic_generator_block, oritech:bio_generator_block, oritech:lava_generator_block, oritech:fuel_generator_block, oritech:big_solar_panel_block]
---

オリテックシステムにおいて、発電機は他のすべての機械やツールが必要とするエネルギー（**RF**）を生成するために使用されます。
...
基本発電機は燃料を燃焼させる仕組みで動作します（炉と同様の原理）。この発電機は**1時間あたり32 RF**を生成します。
...
大型ソーラーパネルは、使用する[マルチブロック式機械コア](../processing/multiblocks)に応じて、**1時間あたり32 RF**から**224 RF**までの範囲で発電可能です。モデルは太陽の動きに合わせて自動的に追従します。動作するのは昼間のみです。
