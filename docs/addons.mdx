...
Addons are activated when the machine is right-clicked, or when a machine is found when the addon is placed. To indicate activation, the pink parts of an addon turn blue when in use.

<Callout variant="warning">
    Addons change color when connected. Blue means connected, pink means disconnected. If an addon is pink, it's not connected to any machine.
</Callout>

To extend the available number of addon slots, you can use machine extenders. These specific addons do not directly influence the machine but allow
