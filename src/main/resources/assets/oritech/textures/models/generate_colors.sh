...
SOURCE_INDUSTRIAL_DOOR_PRIMARY=(
...
IGNORE_LIST=(
    "industrial_door.png"
    "tachyon_absorber.png"
    "cybernetic_augmentation_center.png"
    "big_solar_panel.png"
    "stabilized_enchanter.png"
    "arcane_catalyst.png"
    "pipe_booster.png"
)
...
for img in "${TARGET_FILES[@]}"; do
    ...
    for variant in "${VARIANTS[@]}"; do
        ...
        count_door=${#SOURCE_INDUSTRIAL_DOOR_PRIMARY[@]}
        for (( i=0; i<count_door; i++ )); do
            src="${SOURCE_INDUSTRIAL_DOOR_PRIMARY[$i]}"
            tgt="${TGT_DOOR[$i]}"
            [ -z "$tgt" ] && continue
            args+=("-fill" "$tgt" "-opaque" "$src")
        done
