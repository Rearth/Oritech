    ...
    public void generate(HolderLookup.Provider registryLookup, Consumer<AdvancementHolder> consumer) {
        ...
        var overEnchanted = Advancement.Builder.advancement().parent(arcaneAdvancement)
                                .display(
                                 ...
                                 // The first string used in criterion is the name referenced by other advancements when they want to have 'requirements'
                                 .addCriterion("got_overenchanted", InventoryChangeTrigger.TriggerInstance.hasItems(ItemPredicate.Builder.item().withComponents(DataComponentMatchers.Builder.components().partial(DataComponentPredicates.ENCHANTMENTS, EnchantmentsPredicate.enchantments(List.of(new EnchantmentPredicate(Optional.empty(), MinMaxBounds.Ints.atLeast(10))))).build())))
