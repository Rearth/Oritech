...
import rearth.oritech.client.renderers.blocks.PortableTankRenderer;
 ...
 * <p>
 * <b>Required item model json</b> (e.g. {@code assets/oritech/items/portable_tank.json}): the item must use a
 ...
 * id ({@link #ID}, {@code "oritech:portable_tank"}) and carries the {@code "model"} field consumed by
 ...
 *     "base": "oritech:block/portable_tank",
 *     "model": {
 *       "type": "oritech:portable_tank",
 *       "model": "oritech:portable_tank_body"
 *     }
 *   }
 * }
 * }</pre>
 ...
 */
public class PortableTankItemRenderer implements SpecialModelRenderer<PortableTankItemRenderer.TankContents> {

    public static final Identifier ID = Oritech.id("portable_tank");

    private final Identifier tankVisualModelId;

    public PortableTankItemRenderer(Identifier tankVisualModelId) {
    ...
    public void submit(@Nullable TankContents argument, PoseStack matrices, SubmitNodeCollector collector, int light, int overlay, boolean hasFoil, int outlineColor) {
        ...
        if (argument.fluidSprite() != null) {
            PortableTankRenderer.submitTankFluid(collector, matrices, argument.fluidSprite(), argument.fluidColor(), argument.fill(), light, overlay);
        }
    ...
    public record Unbaked(Identifier model) implements SpecialModelRenderer.Unbaked<TankContents> {
        ...
        public SpecialModelRenderer<TankContents> bake(SpecialModelRenderer.BakingContext context) {
            return new PortableTankItemRenderer(this.model);
        }
